...
import { User, Mail, Lock, Phone, Leaf, Home, Search, ArrowRight } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (response.data.user.role === 'seller') router.push('/profile');
      else router.push('/');
    } catch (err: any) {
      console.error(err);
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-32 sm:py-36">
        <div className="w-full max-w-5xl grid lg:grid-cols-2 rounded-[2rem] overflow-hidden border border-border/80 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.15)] bg-card">
          ...
          <div className="relative hidden lg:block bg-[#0F172A]">
            <img
              src="https://images.unsplash.com/photo-1416331108676-a22ccb276e35?w=900&h=1200&fit=crop"
              alt="Green neighbourhood"
            ...
            <div className="relative z-10 flex flex-col justify-between h-full p-10 text-white">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {[
                    { icon: Search, text: 'Browse verified, transparent listings' },
                    { icon: Home, text: 'Save homes and track your inquiries' },
                  ].map(({ icon: Icon, text }) => (
          ...
          <div className="p-8 sm:p-10">
            <div className="mb-6">
              ...
              <p className="text-sm font-semibold text-slate-400">Join DreamHome and find your perfect place</p>
            </div>

            {error && (
              <div className="mb-5 p-4 bg-rose-50 border border-rose-100 rounded-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Sign up as</label>
                  ...
                  {['buyer', 'seller'].map((r) => (
                    <button
                      ...
                      className={`py-2 rounded-xl text-xs font-bold capitalize transition-all duration-300 ${
                        formData.role === r ? 'bg-[#0F172A] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'
                      }`}
                    >
                      {r}
                    </button>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="John Doe" className={inputBase} />
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} required placeholder="+91 00000 00000" className={inputBase} />
                  </div>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="password" name="password" value={formData.password} onChange={handleChange} required minLength={6} placeholder="••••••••" className={inputBase} />
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Confirm</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} required placeholder="••••••••" className={inputBase} />
                  </div>
                </div>
              </div>
              ...
              >
                {isLoading ? 'Creating account…' : <>Create account <ArrowRight className="w-3.5 h-3.5" /></>}
              </Button>
            </form>
            ...
            <div className="mt-6 pt-6 border-t border-border/80">
              <p className="text-center text-xs text-slate-400 font-semibold">
                ...
                <Link href="/auth/login" className="text-accent hover:text-slate-800 font-bold uppercase tracking-wider ml-1">
                  Sign in
                </Link>
              </p>
            </div>
